...
ip_log_start()
{
	...
	do
	    ...
	    echo "--> capture FLIP packets of fi0@$nd <--"
    	#ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump  -n -e -vv -ttt -i $BOND1 ether proto 0x8901 -w $nd@$BOND1.cap" &
        ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump  -n -e -vv -ttt -i fi0 ether proto 0x8901 -w $nd@fi0.cap" &
        echo "--> capture FLIP packets of fi1@$nd <--"
        ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump  -n -e -vv -ttt -i fi1 ether proto 0x8901 -w $nd@fi1.cap" &
    	
        echo "--> MOnster messages on $nd <--"
    	#ssh  -q -o "StrictHostKeyChecking no" $nd "monster -f 0a25 > ipn_msg_mon.log" &
        ssh  -q -o "StrictHostKeyChecking no" $nd "monster -f 0a25 > ${nd}_ipn_msg_mon.bin" &
	done
    fi
}
...
tdm_log_start()
{
	...
	do
	    #echo "--> capture FLIP packets of $FI@$nd <--"
	    #ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump -i $FI ether proto 0x8901 -nn -xx -s 0 -w $nd@$FI.cap" &
	    echo "--> capture FLIP packets on $BOND1@$nd <--"
	    ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump  -n -e -vv -ttt -i $BOND1 ether proto 0x8901 -w $nd@$BOND1.cap" &
	    echo "--> Monster messages on $nd <--"
	    ssh  -q -o "StrictHostKeyChecking no" $nd "monster -f 0a23,0a31 > ${nd}_msg_mon.bin &" &
	done
    fi
}
...
tcu_log_start()
{
	...
	do
    	...
    	echo "--> Get all the dsp statistic <-- !!!!!Note: not done yet!!!"
...
common_log_start()
{
        ...
        echo "--> start monster capture @CLA-0 <--"
    monster -f 0744,05f1,04b4,05fa,05ec,0646,0656,05f3,05e4,0683,0612,04a8,05fb,063a,06b0,0909,0a21,048e,05f2,0175,0574,09d0,09ce,0a26,0a27,09cf,00db,0680,06b1,041d,0399,0310,06a9 -b ./$LOG_PATH/CLA-0.bin &
        ...
        echo "--> start monster capture @CLA-1 <--"
        #ssh  -q -o "StrictHostKeyChecking no" CLA-1 "rm -rf *.bin"
        ssh  -q -o "StrictHostKeyChecking no" CLA-1 "monster -f 0744,05f1,04b4,05fa,05ec,0646,0656,05f3,05e4,0683,0612,04a8,05fb,063a,06b0,0909,0a21,048e,05f2,0175,0574,09d0,09ce,0a26,0a27,09cf,00db,0680,06b1,041d,0399,0310,06a9 -b /root/CLA-1.bin " &
    ...
    then
        ...
        echo "-->start tcpdump capture for sctp_h248 on CLA-0 fi0<--"
        #tcpdump -n -e -vv -ttt -i fi0 -w CLA-0-fi0.cap &
        tcpdump -i fi0 -xx -nn -s 0 -w ./$LOG_PATH/CLA-0-fi0.cap &
        echo $! > ./$PID_PATH/h248_mon_0.pid


        echo "-->start tcpdump capture for sctp_h248 on CLA-0 fi1"
        #tcpdump -n -e -vv -ttt -i fi1 -w CLA-0-fi1.cap &
        tcpdump -i fi1 -xx -nn -s 0 -w ./$LOG_PATH/CLA-0-fi1.cap &
        echo $! > ./$PID_PATH/h248_mon_1.pid

        echo "--> start tcpdump capture for sctp_h248 on CLA-1 <--"
        ...
        ssh  -q -o "StrictHostKeyChecking no" CLA-1 "tcpdump -i fi0 -xx -nn -s 0 -w CLA-1-fi0.cap " &
        ssh  -q -o "StrictHostKeyChecking no" CLA-1 "tcpdump -i fi1 -xx -nn -s 0 -w CLA-1-fi1.cap " &
    ...
    then
        for nd in ${isu_nodes[*]}
        do
            echo "--> start tcpdump capture for sctp_h248 on  @$nd <--"
            ...
            ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump -i fi0 -xx -nn -s 0 -w $nd-fi0.cap " &
            ssh  -q -o "StrictHostKeyChecking no" $nd "tcpdump -i fi1 -xx -nn -s 0 -w $nd-fi1.cap " & 
        done
    fi    
