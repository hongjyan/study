...
wait_trigger()
cmd = 'hascli -ln ' + node[0]
log(cmd)
output = cli(cmd)
log(output)
cmd = 'hascli -un ' + node[0]
log(cmd)
output = cli(cmd)
log(output)
send_trigger()
...
if g_debug:
    import time
    time.sleep(60) #wait 60s for TDM related service start up

activeTDMNIP = lib_MO.class_MOState(node[0])
if not activeTDMNIP.MO_is_OK()[0]:
    log("TDMNIP related service haven't been in service, stop test case")
