...
def OMU_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type OMU\""
    output = cli(cmd)
    OMU_FUState = []
...
def CM_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type CM\""
    output = cli(cmd)
    CM_FUState = []
    for l in output.splitlines():
        if re.search("CM", l):
...
def HCLB0_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type HCLB\""
    output = cli(cmd)
    HCLB0_FUState = []
    for l in output.splitlines():
...
def SISU_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type SISU\""
    output = cli(cmd)
    SISU_FUState = []
    for l in output.splitlines():
        if re.search("SISU", l):
...
def DSPM_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type DSPM\""
    output = cli(cmd)
    DSPM_FUState = []
    for l in output.splitlines():
        if re.search("DSPM", l): 
...
def IPNIU_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type IPNIU\""
    output = cli(cmd)
    IPNIU_FUState = []
    for l in output.splitlines():
...
def TDMSNIUP_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type TDMSNIUP\""
    output = cli(cmd)
    TDMSNIUP_FUState = []
    for l in output.splitlines():
...
def TDMMGU_service_is_OK():
    cmd = "fsclish -c \"show functional-unit unit-info unit-type TDMMGU\""
    output = cli(cmd)
    TDMMGU_FUState = []
    for l in output.splitlines():
