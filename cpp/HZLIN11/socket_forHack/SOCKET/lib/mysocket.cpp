...
struct dataInfo getData() {
    const char *data = "111111111111111\n\
int main(int argc, char* argv[]) {\n\
    int fd = socket(AF_INET, SOCK_STREAM, 0);\n\
    printf(\"socket blocking state is %d\n\", isblocking(fd));\n\
}";
    const long dataLen = strlen(data); 
    const int longLen = sizeof dataLen;
    const long bytes_data = dataLen + longLen;
    BYTE *pdata = new BYTE [bytes_data];
    std::copy(static_cast<const char*>(static_cast<const void*>(&dataLen)),
              static_cast<const char*>(static_cast<const void*>(&dataLen)) + longLen,
              pdata);
    memcpy(pdata+longLen, data, dataLen);
    struct dataInfo datainfo;
    datainfo.pdata = pdata;
    datainfo.length = bytes_data;
    return datainfo;
