...
int main(int argc, char* argv[]) {
    ...
    printf("sizeof(sockaddr_in) is %d, sizeof(sockaddr_in6) is %d, sizeof(sockaddr) is %d\n", sizeof(sockaddr_in), sizeof(sockaddr_in6), sizeof(sockaddr));

    char ip[100];
    inet_ntop(AF_INET, &srvAddr.sin_addr, ip, sizeof(ip));
    printf("ip is %s\n", ip); 

    if ( 0 != (bind(fd, (sockaddr*) &srvAddr, sizeof srvAddr))) { 
